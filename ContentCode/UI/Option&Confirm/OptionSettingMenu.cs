...
namespace lLCroweTool.UI.Option
{
    public class OptionSettingMenu : MonoBehaviour
    {
        ...
        public Toggle fullDisplayToggle;
        ...
        //키값세팅관련
        public Button showInputKeySettingButton;//키값세팅메뉴를 열기위한 버튼
        ...
        protected void Awake()
        {
            ...
            fullDisplayToggle.onValueChanged.AddListener(SetFullScreenToggle);
            ...
            audioToggle.onValueChanged.AddListener(SetAudioToggle);
        ...
        private static void SetSliderInProgressor(Slider slider, int min, int max, float curValue)
        {
            ...
            if (!slider.TryGetComponent(out UIBar_Base bar))
            {
                return;
            }

            //프로그래서 처리
            slider.onValueChanged.AddListener(bar.SetCurValue);
            bar.InitUIBar(min,max,curValue);
