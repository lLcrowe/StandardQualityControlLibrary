...
namespace lLCroweTool.ScoreSystem.UI
{
    public class MVPScoreBoard : MonoBehaviour
    {
        ...
        public class MVPScoreSort : ValueSort, IComparer<ScoreTargetData>, IComparer<MVPScoreData>
        {
            public MVPScoreSort(bool isAsc) : base(isAsc)
            {
            }

            public int Compare(ScoreTargetData x, ScoreTargetData y)
            {
                return Compare(x.scoreTypeForScore, y.scoreTypeForScore);
            }

            public int Compare(MVPScoreData x, MVPScoreData y)
            {
                return Compare(x.totalScore, y.totalScore);
            }
        }


        private MVPScoreSort scoreTargetDataSort = new MVPScoreSort(false);
        ...
        public void RegisterScoreTarget(IScoreUnit unitObject, ScoreTarget scoreTarget)
        {
            ...
            //여기서 스코어타겟 점수는 정렬시켜줌
            mVPScoreInfo.scoreTargetDataList.Sort(scoreTargetDataSort);
        ...
        public IScoreUnit ShowMVP()
        {
            //총점을 정렬
            mVPScoreInfoList.Sort(scoreTargetDataSort);
            ...
            mvpUnitNameText.text = $"{highScoreUnit.GetUnitName()}";//유닛이름
